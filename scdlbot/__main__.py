...
DEFAULT_CHAT_STORAGE_PATH = "/tmp/scdlbot.pickle"
DEFAULT_DL_DIR_PATH = "/tmp/scdlbot"
CHAT_STORAGE = os.path.expanduser((os.getenv("CHAT_STORAGE") or DEFAULT_CHAT_STORAGE_PATH).strip() or DEFAULT_CHAT_STORAGE_PATH)
DL_DIR = os.path.expanduser((os.getenv("DL_DIR") or DEFAULT_DL_DIR_PATH).strip() or DEFAULT_DL_DIR_PATH)
CHAT_STORAGE_FALLBACK = os.path.expanduser((os.getenv("CHAT_STORAGE_FALLBACK") or DEFAULT_CHAT_STORAGE_PATH).strip() or DEFAULT_CHAT_STORAGE_PATH)
DL_DIR_FALLBACK = os.path.expanduser((os.getenv("DL_DIR_FALLBACK") or DEFAULT_DL_DIR_PATH).strip() or DEFAULT_DL_DIR_PATH)
...
NO_FLOOD_CHAT_IDS = _env_int_list("NO_FLOOD_CHAT_IDS", default=[0])
...
if "WHITELIST_DOMAINS" in os.environ:
    WHITELIST_DOMAINS = set(x.strip() for x in os.getenv("WHITELIST_DOMAINS").split(",") if x.strip())
BLACKLIST_DOMAINS = {}
if "BLACKLIST_DOMAINS" in os.environ:
    BLACKLIST_DOMAINS = set(x.strip() for x in os.getenv("BLACKLIST_DOMAINS").split(",") if x.strip())
WHITELIST_CHATS = set(_env_int_list("WHITELIST_CHATS"))
BLACKLIST_CHATS = set(_env_int_list("BLACKLIST_CHATS"))
...
async def persistence_info_command_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"- Storage mode: {'ephemeral (/tmp)' if CHAT_STORAGE.startswith('/tmp/') or CHAT_STORAGE == '/tmp' else 'configured path'}",
...
def _ensure_writable_directory(path: pathlib.Path) -> bool:
    try:
        path.mkdir(parents=True, exist_ok=True)
        probe_name = f".write_probe_{os.getpid()}"
...
def _resolve_storage_file_path(*, configured_path: str, fallback_path: str, label: str) -> str:
    configured = pathlib.Path(configured_path).expanduser()
    if _ensure_writable_directory(configured.parent):
        return str(configured)
    fallback = pathlib.Path(fallback_path).expanduser()
    if _ensure_writable_directory(fallback.parent):
        logger.warning(
            "%s path is not writable: %s; falling back to %s",
            label,
            configured,
            fallback,
            extra={"event": "storage_fallback", "status": "warn"},
        )
        return str(fallback)
    logger.warning(
        "%s path and fallback path are not writable: configured=%s fallback=%s",
        label,
        configured,
        fallback,
        extra={"event": "storage_fallback", "status": "error"},
    )
    return str(configured)


def _resolve_storage_dir_path(*, configured_path: str, fallback_path: str, label: str) -> str:
    configured = pathlib.Path(configured_path).expanduser()
...
def apply_cloud_storage_fallbacks() -> None:
    """Resolve storage paths for ephemeral cloud runtimes."""
    global CHAT_STORAGE, DL_DIR
    CHAT_STORAGE = _resolve_storage_file_path(
        configured_path=CHAT_STORAGE,
        fallback_path=CHAT_STORAGE_FALLBACK,
        label="CHAT_STORAGE",
    )
    DL_DIR = _resolve_storage_dir_path(
        configured_path=DL_DIR,
        fallback_path=DL_DIR_FALLBACK,
        label="DL_DIR",
    )
    storage_ephemeral = CHAT_STORAGE.startswith("/tmp/") or CHAT_STORAGE == "/tmp"
    downloads_ephemeral = DL_DIR.startswith("/tmp/") or DL_DIR == "/tmp"
    if storage_ephemeral or downloads_ephemeral:
        logger.info(
            "ephemeral storage mode active chat_storage=%s dl_dir=%s",
            CHAT_STORAGE,
            DL_DIR,
            extra={"event": "ephemeral_storage_mode", "status": "ok"},
        )
...
def get_parsed_runtime_config() -> dict:
    return {
        ...
        "CHAT_STORAGE_FALLBACK": CHAT_STORAGE_FALLBACK,
        "DL_DIR": DL_DIR,
        "DL_DIR_FALLBACK": DL_DIR_FALLBACK,
        "WORKERS": WORKERS,
        "EXECUTOR_KIND": EXECUTOR_KIND,
...
def log_startup_summary() -> None:
    summary = {
        ...
        "search_choice_ttl_seconds": SEARCH_CHOICE_TTL_SECONDS,
        "persistence_ephemeral_ttl_seconds": PERSISTENCE_EPHEMERAL_TTL_SECONDS,
        "ephemeral_storage": CHAT_STORAGE.startswith("/tmp/") or CHAT_STORAGE == "/tmp" or DL_DIR.startswith("/tmp/") or DL_DIR == "/tmp",
